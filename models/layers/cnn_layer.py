...
https://github.com/rc_model/tensor2tensor/
...
def mask_logits(inputs, mask, mask_value = -1e30):
    ...
    return inputs + mask_value * (1 - mask) #[batch, p_len, q_len] + [batch, p_len, 1]
...
def optimized_trilinear_for_attention(args, input_keep_prob=1.0,
    ...
    arg_size = arg0_shape[2] #embed_size
    dtype = args[0].dtype
    ...
    with tf.variable_scope(scope):
        ...
        c_maxlen = tf.shape(args[0])[1]
        q_maxlen = tf.shape(args[1])[1]
        subres0 = tf.tile(dot(droped_args[0], weights4arg0), [1, 1, q_maxlen])#[batch, p_len, emb_size]*[emb_size, 1]->[batch, p_len, 1]
        subres1 = tf.tile(tf.transpose(dot(droped_args[1], weights4arg1), perm=(0, 2, 1)), [1, c_maxlen, 1]) #[batch, q_len, emb_size] *[emb_size, 1] - > [batch, p_len, q_len]
        subres2 = batch_dot(droped_args[0] * weights4mlu, tf.transpose(droped_args[1], perm=(0, 2, 1))) #[batch, p_len, emb_size], [batch, emb_size, q_len]
        res = subres0 + subres1 + subres2 #[batch, p_len, q_len]
